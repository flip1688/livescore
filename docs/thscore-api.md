...
- ⚠️ **Timezone ระบุเป็นราย field ตาม docs — field ไหน docs ไม่ระบุ ห้ามเดา** ต้องตรวจกับ payload จริงก่อน
  ...
  - "วันนี้" ของ `livescores.aspx` นับตาม **GMT+0** (00:00–23:59) และ `date` param ของ `schedule/basic.aspx` ก็นับวันแบบ **GMT+0** เช่นกัน (ยืนยัน 2026-07-12)
...
| param | type | required | หมายเหตุ |
|---|---|---|---|
| `date` | string | no | `yyyy-MM-dd`; ย้อนหลังได้ 1 เดือน · ⚠️ **นับวันแบบ GMT+0** ไม่ใช่ GMT+7 (ยืนยัน 2026-07-12: แมตช์ `matchTime` 12-07-2026 04:00 GMT+7 อยู่ใต้ `date=2026-07-11`) — แมตช์ตี 1–7 โมงเช้าบ้านเราตกอยู่ใน date เมื่อวาน, schedule sync เลยต้องดึง เมื่อวาน..พรุ่งนี้ |
...
> ⚠️ **สกอร์ช่วงต่อเวลาไม่อยู่ใน `homeScore`/`awayScore`** (ยืนยันจาก payload จริง เวิลด์คัพรอบ 8 ทีม 2026-07-12): พอเข้าช่วงต่อเวลา `homeScore`/`awayScore` **แช่แข็งที่สกอร์ 90 นาที** — สกอร์สดวิ่งต่อใน `extraExplain.extraHomeScore`/`extraAwayScore` (เป็นสกอร์รวมตั้งแต่เริ่มเกม ไม่ใช่เฉพาะช่วงต่อเวลา) ส่วนจุดโทษอยู่ใน `penHomeScore`/`penAwayScore` — mapper ฝั่งเรา (`applyExtra` ใน `internal/service/mapper.go`) โปรโมตสกอร์รวมขึ้น `home_score`/`away_score` และเก็บรายละเอียดใน field `extra` ของ Match · แถวที่ไม่มีข้อมูลน็อคเอาท์ ส่ง `extraExplain` เป็น `""` ได้ — struct มี unmarshaler รองรับ
